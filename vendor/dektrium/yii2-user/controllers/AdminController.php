...
//        $usersAll = \dektrium\user\models\User::find()->all();
//        $referals = [];
//        foreach ($usersAll as $ua){
//            if($ua->parent_id != 0){
//                foreach ($usersAll as $tmp){if($tmp->id == $ua->parent_id){
//                    $name = $tmp->username;
//                    break;
//                }}
//                $referals[$ua->parent_id] = $name;
//            }
//        }
...
//            'referals' => $referals,
    ...
    public function actionDelete($id)
    {
        if ($id == \Yii::$app->user->getId()) {
            ...
            \Yii::$app->getSession()->setFlash('danger', \Yii::t('user', 'You can not remove administrator account'));
        } else {
            $model = $this->findModel($id);
            $event = $this->getUserEvent($model);

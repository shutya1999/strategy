...
class SitemapGenerator {
     ...
     * @access public
     */
    public $sitemapFileName = "sitemap2.xml";
    /**
     * Name of sitemap index file
     * @var string
     * @access public
     */
    public $sitemapIndexFileName = "sitemap.xml";
    /**
     * Robots file name
     * @var string
     * @access public
     ...
     * @access private
     */
    private $baseURL = 'https://zhyvoedelo.com/ru';
    /**
     ...
     * @access private
     */
    private $basePath = '/var/www/zhyvoedelo/zhyvoedelo.com/web/';
    /**
...
// create object
$sitemap = new SitemapGenerator("https://zhyvoedelo.com/ua");
// add urls
$sitemap->addUrl("https://zhyvoedelo.com/ua", date('c'), 'always', '1');

$sitemap->addUrl("https://zhyvoedelo.com/ua/site/about", date('c'), 'weekly', '0.7');
$sitemap->addUrl("https://zhyvoedelo.com/ua/site/brends", date('c'), 'weekly', '0.7');
$sitemap->addUrl("https://zhyvoedelo.com/ua/site/contacts", date('c'), 'weekly', '0.7');

$sitemap->addUrl("https://zhyvoedelo.com/ua/site/schedule", date('c'), 'daily', '0.8');
$sitemap->addUrl("https://zhyvoedelo.com/ua/site/news", date('c'), 'daily', '0.8');

$sitemap->addUrl("https://zhyvoedelo.com/ua/site/korporativnoye_obucheniye", date('c'), 'daily', '0.9');
$sitemap->addUrl("https://zhyvoedelo.com/ua/site/kouching", date('c'), 'daily', '0.9');
$sitemap->addUrl("https://zhyvoedelo.com/ua/site/otkrytye_treningi_i_master_klassy", date('c'), 'daily', '0.9');



//тренеры
$treners = \app\models\Trener::find()->where(['lang'=>'ua'])->all();
foreach ($treners as $item){
    $sitemap->addUrl("https://zhyvoedelo.com/ua/trener/".$item->url, date('c'), 'weekly', '0.8');
}

//проекты
$projects = \app\models\Project::find()->where(['lang'=>'ua'])->andWhere(['category'=>'trening'])->all();
foreach ($projects as $item2){
    if($item2->inner_url != null){
        $sitemap->addUrl("https://zhyvoedelo.com/ua/learning/".$item2->inner_url, date('c'), 'weekly', '0.8');
    }
}
$projects = \app\models\Project::find()->where(['lang'=>'ua'])->andWhere(['category'=>'couching'])->all();
foreach ($projects as $item3){
    if($item3->inner_url != null){
        $sitemap->addUrl("https://zhyvoedelo.com/ua/learning/".$item3->inner_url, date('c'), 'weekly', '0.8');
    }
}
$projects = \app\models\Project::find()->where(['lang'=>'ua'])->andWhere(['category'=>'corp'])->all();
foreach ($projects as $item4){
    $sitemap->addUrl("https://zhyvoedelo.com/ua/learning/".$item4->url, date('c'), 'weekly', '0.8');
}

//разделы новостей по тегам
$tags = \app\models\Tags::find()->all();
foreach ($tags as $item5){
    $sitemap->addUrl("https://zhyvoedelo.com/ua/category/".$item5->name, date('c'), 'daily', '0.8');
}


//обновить индекс-сайтмап
$updateXml = '<?xml version="1.0" encoding="UTF-8"?>
<sitemapindex xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9                                     http://www.sitemaps.org/schemas/sitemap/0.9/siteindex.xsd">
<sitemap><loc>https://zhyvoedelo.com/sitemap1.xml</loc><lastmod>'.date("Y-m-d").'</lastmod></sitemap><sitemap><loc>https://zhyvoedelo.com/sitemap2.xml</loc><lastmod>'.date("Y-m-d").'</lastmod></sitemap></sitemapindex>';
file_put_contents(Yii::getAlias('@app/web/').'sitemap.xml', $updateXml);

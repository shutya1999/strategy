...
class SitemapGenerator {
     ...
     * @access public
     */
    public $sitemapFileName = "sitemap1.xml";
    /**
...
// create object
$sitemap = new SitemapGenerator("https://zhyvoedelo.com/ru");
// add urls
$sitemap->addUrl("https://zhyvoedelo.com/ru", date('c'), 'always', '1');

$sitemap->addUrl("https://zhyvoedelo.com/ru/site/about", date('c'), 'weekly', '0.7');
$sitemap->addUrl("https://zhyvoedelo.com/ru/site/brends", date('c'), 'weekly', '0.7');
$sitemap->addUrl("https://zhyvoedelo.com/ru/site/contacts", date('c'), 'weekly', '0.7');

$sitemap->addUrl("https://zhyvoedelo.com/ru/site/schedule", date('c'), 'daily', '0.8');
$sitemap->addUrl("https://zhyvoedelo.com/ru/site/news", date('c'), 'daily', '0.8');

$sitemap->addUrl("https://zhyvoedelo.com/ru/site/korporativnoye_obucheniye", date('c'), 'daily', '0.9');
$sitemap->addUrl("https://zhyvoedelo.com/ru/site/kouching", date('c'), 'daily', '0.9');
$sitemap->addUrl("https://zhyvoedelo.com/ru/site/otkrytye_treningi_i_master_klassy", date('c'), 'daily', '0.9');



//тренеры
$treners = \app\models\Trener::find()->where(['lang'=>'ru'])->all();
foreach ($treners as $item){
    $sitemap->addUrl("https://zhyvoedelo.com/ru/trener/".$item->url, date('c'), 'weekly', '0.8');
}

//проекты
$projects = \app\models\Project::find()->where(['lang'=>'ru'])->andWhere(['category'=>'trening'])->all();
foreach ($projects as $item2){
    if($item2->inner_url != null){
        $sitemap->addUrl("https://zhyvoedelo.com/ru/learning/".$item2->inner_url, date('c'), 'weekly', '0.8');
    }
}
$projects = \app\models\Project::find()->where(['lang'=>'ru'])->andWhere(['category'=>'couching'])->all();
foreach ($projects as $item3){
    if($item3->inner_url != null){
        $sitemap->addUrl("https://zhyvoedelo.com/ru/learning/".$item3->inner_url, date('c'), 'weekly', '0.8');
    }
}
$projects = \app\models\Project::find()->where(['lang'=>'ru'])->andWhere(['category'=>'corp'])->all();
foreach ($projects as $item4){
    $sitemap->addUrl("https://zhyvoedelo.com/ru/learning/".$item4->url, date('c'), 'weekly', '0.8');
}

//разделы новостей по тегам
$tags = \app\models\Tags::find()->all();
foreach ($tags as $item5){
    $sitemap->addUrl("https://zhyvoedelo.com/ru/category/".$item5->name, date('c'), 'daily', '0.8');
}
